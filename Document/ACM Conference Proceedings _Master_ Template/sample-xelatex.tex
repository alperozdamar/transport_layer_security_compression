...
The middle capacity link (R1, R2) then varies and tuned with different capacity parameter from 1 to 10 Mbps, reporting each deltatH - deltatL. The link capacity of the two outer (non compression) links on the four node topology was set persistently to 5 Mbps.  Confounding variables across the four simulation nodes were controlled , as the timing between these four simulation types were ultimately comparing to detect the compression link. 
