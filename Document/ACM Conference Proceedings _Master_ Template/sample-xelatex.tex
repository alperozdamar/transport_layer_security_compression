...
\title{Smart Compression Point to Point Algorithm for  Traffic Control}
...
system \cite{campbell1974portable}\cite{bertsekas1987chapter}\cite{deng2012online} for home automation \cite{teymourzadeh2013smart} and industry application.  Several research works were conducted to transfer packets 
in high speed rate possibles and yet it is a challenging task.  In this research project, design and implementation of a smart network link with compression feature is considered. The designed topology is smart to detect if compression occurred in high traffic condition while the network when capacity link is the bottle-neck link. To validate the compression functionality, two set of packet trains were generated and transferred with the different data rate on the transfer path. In this research work, the focus was taken on the data compression approach to fully utilize 
the bandwidth when point to point network is a bottle neck. The design was defined under Microsoft  point to point protocol standard and network simulation (NS-3) \cite{zorn1999point} were used for system design and implementation. 
A comprehensive data analysis and evaluation were conducted to observe network efficiency when compression is activated 
\end{abstract}
...
As discussed earlier, the network topology was implemented in the Point-to-Point Protocol (PPP , RFC 1661) \cite{point1991autonet} model library. The native PPP link is assumed to be established and authenticated at all the time. At first,4 nodes topology was designed that failed to send high entropy and low entropy packet trains concurrently for measurement purpose and automation. Hence, the 5 nodes topology with 2 senders were replaced with the original design. 
