...
Figure ?: The specified method to implement for assembling compressed packets, as specified in RFC 1974[?].
...
\end{itemize}
 ...
 In the other hand, RECEIVE function decompressed the packet payload data after removing all necessary header file.  The designed system is smart to detect and target only compressed packet for decompression process. Figure ? shows the headr file involve with the arriving packet and the compression process on the packet. 
 
 Figure ?
 
 Second UDP client is incharge to generate high entropy trained packet and run it through the network. The same process will take place for high entropy trained packet. UDP client 
